...
// ✅ Se agregan StudentPackage y Package a los imports
  ...
  try {
    ...
    const exams = await Exam.findAll({
      where: {
        ...
        status: { [Op.in]: ['assigned', 'completed', 'approved', 'rejected'] }
      },
  ...
  try {
    ...
    try {
      examAssignment = await ExamAssignment.findOne({
        where: { examId: id, teacherId: teacher.id }
      });
    } catch (err) {
...
router.post('/exams/:examId/questions/:questionId/answer', hasRole(['teacher']), async (req, res) => {
  ...
  try {
    ...
    try {
      examAssignment = await ExamAssignment.findOne({
        where: { examId, teacherId: teacher.id, status: 'assigned' }
      });
    } catch (err) {
    ...
    let answer = null;
    try {
      answer = await ExamAnswer.findOne({ where: { examId, questionId, teacherId: teacher.id } });
    } catch (err) {
...
router.patch('/exams/:id/submit', hasRole(['teacher']), async (req, res) => {
  ...
  try {
    ...
    if (examAssignment && typeof examAssignment.update === 'function') {
      try {
        await examAssignment.update({ status: 'completed', completedAt: new Date() }, { transaction });
      } catch (err) {
  ...
  try {
    const result = await sequelize.transaction(async (t) => {
      ...
      const teacher = await Teacher.create({
        ...
        workHours, breakHours, specialties,
        maxStudentsPerDay, isCoordinator, workingDays
      }, { transaction: t });
  ...
  try {
    ...
    await sequelize.transaction(async (t) => {
      await teacher.update({
        firstName, lastName, phone,
        workHours, breakHours, specialties,
        maxStudentsPerDay, isCoordinator, active, workingDays
      }, { transaction: t });

      if (isCoordinator !== undefined) {
        await User.update(
          { role: isCoordinator ? 'coordinator' : 'teacher' },
          { where: { id: teacher.userId }, transaction: t }
        );
      }
    });
  ...
  try {
    ...
    try {
      ...
      await TeacherStudent.update(
        { active: false },
        { where: { teacherId: teacher.id, active: true }, transaction: t }
      );
...
// =====================================================================
// ✅ RUTA CORREGIDA: Get teacher schedule
// Cambios:
//   1. Se eliminaron StudentPackage y Package del include (no están asociados a Teacher)
//   2. Se lee weeklySchedule directamente de TeacherStudent (confiable tras el fix del POST /schedule)
//   3. Se normaliza fixedSchedule para que siempre tenga { day, hour, startTime, endTime }
// =====================================================================
// REEMPLAZA SOLO la ruta GET /:id/schedule en tu teachers.js con este codigo
...
router.get('/:id/schedule', verifyToken, hasRole(['admin', 'coordinator', 'teacher']), async (req, res) => {
  ...
  try {
    ...
    console.log('Step 1 - teacherId:', teacherId);

    const teacher = await Teacher.findByPk(teacherId);
    console.log('Step 2 - teacher:', teacher ? 'FOUND' : 'NOT FOUND');
    ...
    console.log('Step 3 - timezone:', userTimezone);

    // Paso 1: estudiantes con weeklySchedule
    console.log('Step 4 - students query...');
    ...
    try {
      ...
      console.log('Step 4 OK - count:', studentsWithSchedule.length);
    } catch (err) {
      console.error('Step 4 FAILED:', err.message, err.stack);
      throw err;
    }

    const assignedStudents = studentsWithSchedule.map(student => {
      const pivot = student.teachers && student.teachers.length > 0
        ? student.teachers[0].TeacherStudent
        : null;
      let rawSchedule = [];
      if (pivot && pivot.weeklySchedule) {
        try {
          rawSchedule = typeof pivot.weeklySchedule === 'string'
            ? JSON.parse(pivot.weeklySchedule)
            : pivot.weeklySchedule;
        } catch (e) { rawSchedule = []; }
      }
      const fixedSchedule = (Array.isArray(rawSchedule) ? rawSchedule : []).map(slot => {
        const hour = slot.hour !== undefined
          ? parseInt(slot.hour)
          : parseInt((slot.startTime || '0:00').split(':')[0]);
        return {
          day: slot.day,
          hour,
          startTime: slot.startTime || `${String(hour).padStart(2,'0')}:00`,
          endTime: slot.endTime || `${String(hour+1).padStart(2,'0')}:00`
        };
      });
      return { id: student.id, fullName: `${student.name} ${student.surname}`, fixedSchedule };
    });
    console.log('Step 5 - assignedStudents mapped:', assignedStudents.length);
    ...
    // Paso 3: actividades
    console.log('Step 6 - activities query...');
    let activities = [];
    try {
      activities = await TeacherActivity.findAll({
        where: {
          teacherId: teacher.id,
      ...
      console.log('Step 6 OK - activities:', activities.length);
    } catch (err) {
      console.error('Step 6 FAILED:', err.message, err.stack);
      throw err;
    }
    ...
    // Paso 4: clases
    console.log('Step 7 - classes query...');
    ...
    try {
      classesWithStudents = await Class.findAll({
        ...
        include: [{
          model: Student,
          as: 'students',
          through: { model: StudentClass, attributes: ['status'] },
          attributes: ['id', 'name', 'surname'],
          required: false
        }],
      ...
      console.log('Step 7 OK - classes:', classesWithStudents.length);
    } catch (err) {
      console.error('Step 7 FAILED:', err.message, err.stack);
      throw err;
    }
    ...
    console.log('Step 8 - rescheduled query...');
    ...
    try {
      ...
      console.log('Step 8 OK - rescheduled:', rescheduledClasses.length);
    } catch (err) {
      console.error('Step 8 FAILED:', err.message, err.stack);
      throw err;
    }
    ...
    const formattedRescheduled = rescheduledClasses.map(r => {
      ...
      if (rJson.oldClass) {
        const t = timezoneUtils.convertFromAdminToUserTimezone(rJson.oldClass.date, rJson.oldClass.startTime, userTimezone);
        rJson.oldClass.userDate = t.date;
        rJson.oldClass.userStartTime = t.time;
      }
      if (rJson.newClass) {
        const t = timezoneUtils.convertFromAdminToUserTimezone(rJson.newClass.date, rJson.newClass.startTime, userTimezone);
        rJson.newClass.userDate = t.date;
        rJson.newClass.userStartTime = t.time;
      }
      return rJson;
    });
    ...
    return res.json({
      ...
      rescheduledClasses: formattedRescheduled,
      ...
      include: [
        {
          ...
          as: 'teachers',
          where: { id: teacherId },
          through: { where: { active: true }, attributes: ['assignedDate', 'notes'] }
        },
...
router.post('/:id/students', verifyToken, hasRole(['admin']), async (req, res) => {
  try {
    ...
    const existingAssignment = await TeacherStudent.findOne({
      where: { studentId, active: true, teacherId: { [Op.ne]: teacherId } }
    });

    if (existingAssignment) {
      const existingTeacher = await Teacher.findByPk(existingAssignment.teacherId);
      const teacherName = existingTeacher ? 
        `${existingTeacher.firstName} ${existingTeacher.lastName}` : 'another teacher';
    ...
    const existingTeacherAssignment = await TeacherStudent.findOne({
      where: { studentId, teacherId, active: true }
    });
    ...
    const inactiveAssignment = await TeacherStudent.findOne({
      where: { studentId, teacherId, active: false }
    });
...
router.delete('/:id/students/:studentId', verifyToken, hasRole(['admin']), async (req, res) => {
  try {
    ...
    const assignment = await TeacherStudent.findOne({
      where: { teacherId, studentId, active: true }
    });
...
router.post('/:id/activities', verifyToken, hasRole(['admin', 'coordinator', 'teacher']), async (req, res) => {
  try {
    ...
    const activity = await TeacherActivity.create({
      teacherId, title, description, date, startTime, endTime,
      type, deadline, notes, timezone: appTimezone, status: 'scheduled'
    });
...
// Get teacher's tasks (by teacher ID)
...
// Get teacher's exams (by teacher ID)
router.get('/:id/exams', verifyToken, hasRole(['admin', 'coordinator', 'teacher']), async (req, res) => {
  try {
    ...
    if (status) {
      if (status === 'pending') {
        whereConditions.status = { [Op.in]: ['assigned', 'draft'] };
      } else if (status === 'completed') {
        whereConditions.status = { [Op.in]: ['completed', 'approved', 'rejected'] };
      } else {
        whereConditions.status = status;
      }
    }
    ...
    const examsData = await Exam.findAll({
      ...
      include: [
        ...
        { model: ExamAssignment, as: 'assignments', where: { teacherId }, required: false }
      ],
      order: [['createdAt', 'DESC']]
    });
...
router.get('/:id/history', verifyToken, hasRole(['admin', 'coordinator', 'teacher']), async (req, res) => {
  try {
    ...
    const tasks = await Task.findAll({
      where: { assignedTo: teacher.id, status: { [Op.in]: ['completed', 'reviewed'] } },
      include: [{ model: Teacher, as: 'coordinator', attributes: ['id', 'firstName', 'lastName'] }],
    ...
    const exams = await Exam.findAll({
      where: { assignedTo: teacher.id, status: { [Op.in]: ['completed', 'approved', 'rejected'] } },
      include: [
        { model: Teacher, as: 'coordinator', attributes: ['id', 'firstName', 'lastName'] },
    ...
    const examAssignments = await ExamAssignment.findAll({
      where: { teacherId: teacher.id, status: { [Op.in]: ['completed', 'approved', 'rejected'] } },
      include: [{
        model: Exam,
        as: 'exam',
        include: [
    ...
    const directExams = exams.map(exam => ({
      ...
      dueDate: exam.dueDate,
      status: exam.status,
      result: exam.status === 'completed' ? 'Completed' : exam.status === 'approved' ? 'Approved' : 'Rejected',
      totalQuestions: exam.questions ? exam.questions.length : 0,
    ...
    const assignedExams = examAssignments.map(assignment => {
      ...
      return {
        ...
        result: assignment.status === 'completed' ? 'Completed' : assignment.status === 'approved' ? 'Approved' : 'Rejected',
        totalQuestions: exam.questions ? exam.questions.length : 0,
        coordinator: exam.coordinator ? `${exam.coordinator.firstName} ${exam.coordinator.lastName}` : 'System',
    ...
    const formattedExams = [...directExams, ...assignedExams].filter(exam => {
      if (examIds.has(exam.id)) return false;
      examIds.add(exam.id);
      return true;
    });
    
    return res.json({ tasks: formattedTasks, exams: formattedExams });
  } catch (error) {
    console.error('Error fetching teacher history:', error);
    return res.status(500).json({ error: 'Failed to fetch history data', tasks: [], exams: [] });
  }
});
...
router.get('/:id/classes', verifyToken, hasRole(['admin', 'coordinator', 'teacher']), async (req, res) => {
  try {
      ...
      include: [
        {
          ...
          as: 'students',
          attributes: ['id', 'name', 'surname'],
          through: { model: StudentClass, attributes: ['status', 'originalClassId', 'notes'] }
        },
        {
    ...
    const filteredClasses = classes.filter(cls => {
      if (!cls.students || cls.students.length === 0) return true;
    ...
    const classesWithUserTimezone = filteredClasses.map(cls => {
      const userTime = timezoneUtils.convertFromAdminToUserTimezone(cls.date, cls.startTime, userTimezone);
      const userEndTime = timezoneUtils.convertFromAdminToUserTimezone(cls.date, cls.endTime, userTimezone);
      return {
        ...cls.toJSON(),
...
// Get rescheduled classes assigned to this teacher
  ...
  try {
    ...
    let whereClause = {
      [Op.or]: [
        ...
        { differentTeacher: true, '$newClass.teacherId$': teacherId }
      ]
    };
    ...
    const rescheduledClasses = await RescheduleClass.findAll({
      where: whereClause,
      include: [
        { model: Class, as: 'newClass', include: [{ model: Student, as: 'students', attributes: ['id', 'name', 'surname'] }] },
        { model: Class, as: 'oldClass' },
        { model: Student, as: 'student' },
    ...
    const formattedClasses = rescheduledClasses.map(reschedule => {
      ...
      const studentName = reschedule.student ? 
        `${reschedule.student.name} ${reschedule.student.surname || ''}`.trim() : 'Unknown Student';
      const oldTeacherName = reschedule.oldTeacher ? 
        `${reschedule.oldTeacher.firstName} ${reschedule.oldTeacher.lastName || ''}`.trim() : 'Unknown Teacher';
      const newTeacherName = reschedule.newTeacher ? 
        `${reschedule.newTeacher.firstName} ${reschedule.newTeacher.lastName || ''}`.trim() : 'Current Teacher';
...
router.post('/:id/exams/:examId/submit', verifyToken, hasRole(['teacher']), async (req, res) => {
  ...
  try {
    ...
    const exam = await Exam.findOne({
      where: { id: examId, assignedTo: teacherId },
    ...
    if (answers && Array.isArray(answers) && answers.length > 0) {
      ...
      for (const answer of answers) {
        if (!questionIds.includes(answer.questionId)) {
          await transaction.rollback();
          return res.status(400).json({ error: `Answer provided for question ID ${answer.questionId} that doesn't belong to this exam` });
        }
        await ExamAnswer.create({
          examId, questionId: answer.questionId, teacherId,
...
router.post('/:id/classes/:classId/record', verifyToken, hasRole(['teacher']), async (req, res) => {
  try {
    ...
    await classRecord.update({
      status: 'completed',
      breathing, warmup, vocalization, observations,
...
router.get('/:id/available-slots', verifyToken, async (req, res) => {
  try {
    ...
    if (!workingDays.includes(dayOfWeek)) {
      return res.json({ message: 'Teacher does not work on this day', slots: [] });
    }
    
    const dayWorkHours = workHours[dayOfWeek] || [];
    if (dayWorkHours.length === 0) {
      return res.json({ message: 'Teacher has no work hours set for this day', slots: [] });
    }
    ...
    const bookedClasses = await Class.findAll({
      where: {
        ...
        [Op.or]: [
          { teacherId },
          { '$studentClasses.student.teachers.id$': teacherId }
        ]
      },
      include: [{
        model: StudentClass,
        ...
        include: [{
          model: Student,
          as: 'student',
          include: [{ model: Teacher, as: 'teachers', through: { where: { active: true } } }]
        }]
      }]
    });
    ...
    const availableSlots = dayWorkHours.flatMap(workSlot => {
      ...
      while (currentSlotStart.clone().add(classDuration, 'minutes').isSameOrBefore(workEnd)) {
        ...
        if (!isOverlapping) {
          slots.push({ start: currentSlotStart.format('HH:mm:ss'), end: currentSlotEnd.format('HH:mm:ss'), date });
        }
...
router.get('/available-slots', verifyToken, async (req, res) => {
  try {
    ...
    if (!teachers || teachers.length === 0) {
      return res.json({ message: 'No active teachers found', teachers: [] });
    }
